...
if (typeof window !== 'undefined' && typeof EventSource !== "undefined") {
    var source = new EventSource("https://encom-boardroom-dashboard.pages.dev/events");
    var knownLocations = new Set();
    
    // Pre-fill known locations from static data array
    data.forEach(function(item) {
        knownLocations.add(item.lat + "," + item.lng);
    });

    source.onmessage = function(event) {
        try {
            var evtData = JSON.parse(event.data);
            if (evtData.stream === "analytics" && evtData.latlon) {
                var lat = evtData.latlon.lat;
                var lng = evtData.latlon.lon;
                var label = evtData.location || "";
                var locKey = lat + "," + lng;
                
                // Only add if we haven't seen this exact location yet
                // This prevents encom-globe from hiding labels due to pin collisions
                if (!knownLocations.has(locKey)) {
                    ...
                    if (window.encomGlobe) {
                        window.encomGlobe.addPin(lat, lng, label);
                    }
                }
            }
        } catch (e) {
